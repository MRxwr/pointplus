...
function getTop30($startDate, $endDate){
	...
	";
	*/
	$sql = "SELECT p.userId, u.username, SUM(p.points) as total_points FROM `predictions` as p JOIN `user` as u ON p.userId = u.id WHERE p.date BETWEEN '{$startDate}' AND '{$endDate}' GROUP BY p.userId ORDER BY `total_points` DESC LIMIT 50;
	";
	if($result = $dbconnect->query($sql)){
		while($row = $result->fetch_assoc() ){
			$array[] = $row;
