...
<div class="pull-left">
<h6 class="panel-title txt-dark">Team Details</h6>
</div>
...
<form action="?page=teams" method="post" enctype="multipart/form-data">
...
<select class="form-control" name="leagueId" required>
	...
	if ( $leagues = selectDB("leagues","`status` = '0'") ){
		if ( isset($_GET["edit"]) && $league = selectDB("leagues","`id` = '{$data[0]["leagueId"]}'") ){
...
if ( !isset($_GET["edit"]) ){
$status 		= array('0','1');
$arrayOfTitles 	= array('Active teams','Inactive teams');
...
while ( $row = $result->fetch_assoc() ){
	if ( $league = selectDB("leagues","`id` = '{$row["leagueId"]}'") ){
		$leagueTitle  = direction($league[0]["enTitle"],$league[0]["arTitle"]);
	}
?>
<tr>
...
<a href="?page=teams&edit=1&id=<?php echo $row["id"] ?>" style="margin:3px"><i class="fa fa-edit"></i></a>

<a href="?page=teams&<?php echo $action[$i] . $row["id"] ?>" style="margin:3px"><i class="<?php echo $icon[$i] ?>"></i></a>
