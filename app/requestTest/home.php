...
}elseif( $leaderboard = selectDataDB("`id`,`username`, `name`, `points`","user","`status` = '0' AND `type` = '2' AND `rank` = '0' ORDER BY `rank` ASC LIMIT 3") ){
	$response["leaderboard"] = $leaderboard;
}else{
...
// get rounds matches 
if ( isset($rounds[0]["round"]) && !empty($rounds[0]["round"]) && $matches = selectDB("matches","`status` = '1' AND `round` = {$rounds[0]["round"]} ORDER BY `id` DESC") ){
